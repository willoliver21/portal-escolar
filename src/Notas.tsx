...
import { useNotification } from './NotificationContext'; // <-- Importar o nosso hook

// ... (interfaces continuam as mesmas) ...

interface Turma { id: string; nome: string; }
interface Aluno { id:string; nome: string; }
interface Nota { id: string; aluno_id: string; materia: string; nota: number; data: string; }

export function Notas() {
  const { showToast } = useNotification(); // <-- Usar o hook de notificação
  ...
  useEffect(() => {
    ...
    async function fetchAlunosDaTurma() {
      ...
      setSelectedAlunoId('');
      const { data, error } = await supabase.from('matriculas').select('alunos(id, nome)').eq('turma_id', selectedTurmaId);
      if (error) {
  ...
  useEffect(() => {
    ...
    async function fetchNotasDoAluno() {
      ...
      const { data, error } = await supabase.from('notas').select('*').eq('aluno_id', selectedAlunoId);
      if (error) {
  ...
  async function handleAdicionarNota(e: React.FormEvent) {
    ...
    if (!materia || !notaInput) {
      showToast('Por favor, preencha a matéria e a nota.', 'error');
      return;
    }
    ...
    <div>
      ...
      {turmas.length > 0 ? (
        <>
          ...
          <select id="turma-select" value={selectedTurmaId} onChange={e => setSelectedTurmaId(e.target.value)} >
          ...
          {alunos.length > 0 && !loadingAlunos && (
            <div style={{ marginTop: '20px' }}>
              ...
              <select id="aluno-select" value={selectedAlunoId} onChange={e => setSelectedAlunoId(e.target.value)} >
          ...
          {selectedAlunoId && (
            <div style={{ marginTop: '30px' }}>
              ...
              <form onSubmit={handleAdicionarNota}>
                <input type="text" placeholder="Matéria (ex: Matemática)" value={materia} onChange={e => setMateria(e.target.value)} required />
                <input type="number" placeholder="Nota (0-10)" value={notaInput} onChange={e => setNotaInput(e.target.value)} step="0.1" min="0" max="10" required />
